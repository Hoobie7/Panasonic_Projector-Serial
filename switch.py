"""
Use serial protocol of Panasonic projector to obtain state of the projector.
For more details about this component, please refer to the documentation
at https://home-assistant.io/components/switch.panasonic_projector/
"""
import logging
import re
...
DEFAULT_NAME = 'Panasonic Projector'
DEFAULT_TIMEOUT = 1
...
CMD_DICT = {LAMP:'\x02QPW\x03',
            STATE_ON: '\x02PON\x03',
            STATE_OFF: '\x02POF\x03'}
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    """Connect with serial port and return Panasonic Projector."""
    serial_port = config.get(CONF_FILENAME)
    name = config.get(CONF_NAME)
    timeout = config.get(CONF_TIMEOUT)
    write_timeout = config.get(CONF_WRITE_TIMEOUT)

    add_devices([PanasonicSwitch(serial_port, name, timeout, write_timeout)])


class PanasonicSwitch(SwitchDevice):
    """Represents an Panasonic Projector as a switch."""

    def __init__(self, serial_port, name, timeout, write_timeout, **kwargs):
        """Init of the Panasonic projector."""
        import serial
    ...
    def _write_read_format(self, msg):
        ...
        match = re.search(r'\x02(.+)\x03', awns)
        if match:
